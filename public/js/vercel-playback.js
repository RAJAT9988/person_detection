/* Playback mode — videos loop + simulated analytics (no detection JSON) */
(function () {
  'use strict';
  ...
  async function loadVideoAtIndex(video, index) {
    ...
    for (const src of sources) {
      try {
        ...
        console.warn('[Playback] failed:', src);
      }
    }
  ...
  async function initVercelPlayback(opts = {}) {
    const force = opts.force === true;
    if (!force && !isVercelHost()) return false;
    ...
    try {
      ...
      if (res.ok) {
        ...
        playlist = (data.videos || []).map((v) => ({
          ...v,
          fallbackVideo: v.fallbackVideo || (v.video?.startsWith('http') ? null : `/singapor/${v.id}.avi`),
        }));
      }
    } catch (e) {
      console.warn('[Playback] playlist fetch:', e);
    }
    if (!playlist.length) {
      const useLocal = !isVercelHost();
      playlist = ['at1', 'at2', 'at3'].map((id, i) => ({
        id,
        label: `Camera ${i + 1}`,
        video: useLocal ? `/singapor/${id}_processed.mp4` : `${fallbackBase}/${id}_processed.mp4`,
        fallbackVideo: useLocal ? `/singapor/${id}.avi` : `${fallbackBase}/${id}.avi`,
      }));
    }
    ...
    console.log('[Playback] Simulated analytics — no detection JSON');
    return true;
  }
